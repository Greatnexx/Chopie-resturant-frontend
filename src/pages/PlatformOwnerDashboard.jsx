...
const PlatformOwnerDashboard = () => {
  ...
  const approveRestaurant = async (restaurantId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:8000/api/v1';
      const response = await fetch(`${apiUrl}/platform-owner/restaurants/${restaurantId}/approve`, {
        method: 'PUT',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const data = await response.json();
      if (data.status) {
        setRestaurants(prev => prev.map(r => r._id === restaurantId ? { ...r, isApproved: true, isActive: true } : r));
      }
    } catch (error) {
